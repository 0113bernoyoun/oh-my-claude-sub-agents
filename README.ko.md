...
3. 오케스트레이터 프롬프트를 `.claude/omcsa-agents.md`에 생성하고 `.claude/CLAUDE.md`에 `@import` 참조를 추가
...
| `omcsa init --output <mode>` | 출력 모드 지정: `external` (기본값) 또는 `inline` |
...
| `omcsa refresh --output <mode>` | 재스캔 + 출력 모드 지정: `external` 또는 `inline` |
| `omcsa refresh --maturity <mode>` | 재스캔 + 성숙도 모드 적용 |
| `omcsa apply` | `omcsa.config.json` 수정 후 재적용 |
| `omcsa apply --output <mode>` | 재적용 + 출력 모드 지정: `external` 또는 `inline` |
...
OMCSA는 오케스트레이터 프롬프트를 두 가지 방식으로 출력할 수 있습니다.

| 모드 | 설명 |
|------|------|
| `external` (기본값) | `.claude/omcsa-agents.md`에 프롬프트를 생성하고 CLAUDE.md에 `@omcsa-agents.md` 참조만 추가. CLAUDE.md 크기를 대폭 줄임 (~99% 감소) |
| `inline` | 이전 동작과 동일. 오케스트레이터 프롬프트 전체를 CLAUDE.md 마커 안에 직접 삽입 |
...
# 이전 동작 (inline)
omcsa init --output inline

# 설정 파일에서 지정
# omcsa.config.json의 features.outputMode: "external" 또는 "inline"
```

`--output` 옵션은 `init`, `apply`, `refresh` 명령어에서 사용할 수 있습니다.

---

## 스마트 프롬프트 (성숙도 기반)

OMCSA는 CLAUDE.md를 분석하여 오케스트레이션 성숙도 수준을 판단하고, 프롬프트 상세 수준을 자동 조절합니다.

**기본 동작**: `auto` 모드가 기본값으로, OMCSA가 성숙도를 분석하여 프롬프트 상세도를 자동 조절합니다. 숙련된 사용자에게는 축약된 프롬프트가 생성됩니다.

```
Maturity: MEDIUM (0.42) — Adaptive prompt generated.
```

이전 동작(항상 전체 프롬프트)을 원하면 `--maturity full`을 사용하세요:

```bash
omcsa init --maturity full    # 항상 전체 프롬프트 생성
omcsa refresh --maturity full # 재스캔 시 전체 프롬프트 적용
```
...
유효 모드: `"auto"` (기본값), `"full"`, `"LOW"`, `"MEDIUM"`, `"HIGH"`
...
your-project/
├── .claude/
│   ├── CLAUDE.md              ← @import 참조가 추가됨 (external 모드)
│   ├── omcsa-agents.md        ← 오케스트레이터 프롬프트 (external 모드에서 생성)
...
**External 모드** (기본값) -- `@import` 참조만 포함하여 CLAUDE.md 크기를 대폭 줄입니다:
...
**Inline 모드** (`--output inline`) -- 오케스트레이터 프롬프트 전체가 마커 안에 포함됩니다:
...
Hook 파일, 설정 등록, 모드 유효성, 에이전트 파일, CLAUDE.md 섹션 정합성, 성숙도 분석, external 파일(`omcsa-agents.md`) 정합성 및 CLAUDE.md 크기 경고를 검사합니다.
...
- `.claude/omcsa-agents.md` (external 모드 사용 시)
